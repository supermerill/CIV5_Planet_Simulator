...
-------------------------------------------------------------------------------------------
function avgrootbob(guess, bob, root)
    local retbob = mulbob(root, addbob(bob, divbob(guess, bob)))
    print(tonum(retbob))
    return retbob
end
-------------------------------------------------------------------------------------------
...
-- function nthroot(bob, root)

   -- bob = bit.tobit(tonum(bob))
   -- local bias = bit.lshift(1, 10) - 1
...
function powbob(bob, pow)
    ...
    if sqr ~= 0 then
        if sqr > 0.999 then
            ...
            -- sqr = Round(1 / sqr)
            -- local guess = tobob(nthroot(temp, sqr))
            -- ###### rootbob Method (known working) #####
            sqr = Round(1 / sqr)
            local guess = zerobob
            for n = 0, 8 do
...
function GeneratePlotTypes()
    ...
    local num1 = tobob(a)
    --local num2 = tobob(b)
    local num3 = powbob(num1, 0.5)
    -- local num3 = subbob(num1, Ebob)

    print(tonum(num3))



    -- local bool = comparebob(num1, num2, LT)
    -- print(bool)

    -- Time2 = os.clock()
    -- for n = 1, 500000 do
        -- powbob(num1, 0.33333333)
    -- end

	-- print(string.format("500,000 powbob^2+1/3s in %.3f seconds.", os.clock() - Time2))
